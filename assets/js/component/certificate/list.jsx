...
class CertificateComponent extends Component {
    ...
    render() {
        ...
        const { data, loading } = this.props.CertificateReducer.certificates;
                                        ...
                                        <Row col noMargin s={12}>
                                            <h2>
                                                <Appear
                                                    ...
                                                    <Words animate show={animLvl1}>Certificates</Words>
                                                <Appear className={classes.titleRight} animate show={animLvl1} >
                                                    <FontAwesomeIcon icon={faAngleDoubleLeft} />
                                                </Appear>
                                            </h2>
                                            {
                                                _.map(Certificates, (skill, key) => (
                                                    <Project
                                                        animate
                                                        show={anim.entered}
                                                        key={key}
                                                        ...
                                                        onClick={() => {
                                                            window.open(`/certificate/${skill.id}`);
                                                        }}
                                                        >
                                                            <Appear
                                                                animate
...
const mapDispatchToProps = (dispatch) => ({
    GetCertifiates: () => dispatch(GetCertifiates())
});

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(CertificateComponent));
